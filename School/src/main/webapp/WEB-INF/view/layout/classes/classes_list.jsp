...
<c:choose>
  <c:when test = "${number_of_classess == 0}">
  <TR>
    <TD colspan = "6">
      &nbsp; <FONT size = "4" face = "新宋体" color = "Red"> 学校初建,请尽快设计好班级,以免耽误您赚钱的宏图大业! </FONT>
    </TD>
  </TR>
  </c:when>
  <c:otherwise>
    <c:forEach items = "${classesList}" var = "classes" varStatus = "row">
      <c:choose>
        ...
        <c:otherwise>
          <TR>
            ...
            <TD align = "center">
              ...
              <A href="${contextPath}/del_classes.do?id=${classes.id}"> 删除 </A>
              <A href="${contextPath}/updateClassesLeave/${classes.id}.do"> 关停 </A>
            </TD>
          </TR>
    </c:forEach>
          <TR>
            <TD colspan = "6">
              &nbsp;共有 <FONT size = "5" face = "新宋体" color = "Red"> ${number_of_classess} </FONT>个班
            </TD>
          </TR>
  </c:otherwise>
</c:choose>
