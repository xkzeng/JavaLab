...
public class BookController
{
    ...
    mav.setViewName("thymeleaf/book_list"); //关键点
		return mav;
	}
	
	/**
	 * 使用FreeMarker视图解析器显示页面
	 */
	...
	public ModelAndView showBookListWithFreeMarker()
	{
	  ...
	  //选择映射器:获取映射器接口对象实例;
		...
		mav.addObject("number_of_books", number_of_books);
		mav.addObject("bookList", bookList);
		mav.setViewName("freemarker/book_list"); //关键点
		return mav;
	}
	...
	public ModelAndView showBookListWithJspJstl()
	{
	  ...
	  //选择映射器:获取映射器接口对象实例;
		...
		mav.addObject("number_of_books", number_of_books);
		mav.addObject("bookList", bookList);
		mav.setViewName("jsp/book_list"); //关键点
		return mav;
	}
